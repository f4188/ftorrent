...
class DHT {
	...
	isTokenValid(token) {
		...
		if(this.newToken.hosts || this.oldToken.hosts) //
			return true
		else 
			return false
	...
	async announce(infoHash) {
		...
		while( newNodes.difference(oldNodes) ) { //nodes unchanged

			oldNodes = new Set(nodes)
			...
			//results = [[peers, nodes], ...]
			results = nodes.map(node => node.getPeers()).filter(async result => {
				try {
					await result
					return true
				} catch(error) {
			...
			nodes = nodes.concat(flatten(results.map(result => result[1]).flatten()))

			//if new nodes - try to insert in dht
	...
	findNode(nodeID) { //rename findNodes
	...
	insertNodeInDHT(nodeID) {
		//if(this.dht.getNode(node)) return
	...
	_recvRequest(msg, rinfo) {
		...
		if(pack.y == 'q') {
			...
			case 'get_peers' :
				...
				break
			case 'announce_peers' :
				response = node.respAnnounce(req)
				self._sendResponse(response, node.port, node.host)
				break
			default :
	...
	respAnnounce(request) {
		let token = request.a.token
		if(!this.isTokenValid(token))
			return {'t': request.t, 'y':'e', 'e':[203,"Bad Token"]} 
		//might be backward !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		let port = request.a.implied_port != 0 ? this.port : request.a.port
...
class Node {
	...
	constructor(nodeID, port, host, dht) {
		...
		//!resp15min && (!everResp || !query15min)
		this.bad = false

		this.inDHT = false
		//this.newNode = true
		//this.questionable = false
		//in dict and good, in dict and quest, in dict and bad, new (always good), ques or bad
		
		//this.state = this.STATE.GOOD

		//good, questionable, bad
		//if ansQuery15min then good
		//if everAnsQuery and sentQuery15min then good
	...
	async findNode(targetNodeID) {
		...
		//{t: transactid, y: r, r:{id:targetNodeID, nodes: contactInfo or list of nodes }}
		response = await this._sendRequest(request)
		//if(response) this.resetQueryTimer()

		//this.resetRespQueryTimer()
		compactInfos = parseNodeContactInfos(response.r.nodes)//.match(/.{26}/g)
		nodes = compactInfos.map((info)=> {return this.dht.makeNode(info.slice(0,20), info.slice(24,26), info.slice(20,24))})		
	...
	async getPeers(infoHash) {
		let transactID = crypto.randomBytes(2)
		let request = {'t': transactID, 'y':'q', 'name': 'get_peers', 'a' : {'id': this.myNodeID, 'implied_port': 1,
		'infoHash':infoHash, 'port', this.port, 'token': this.token}}
		response = await this._sendRequest(request)
		//if(response) this.resetQueryTimer()
		//this.resetRespQueryTimer()
		//target id, token, nodes or values
		//this.token = response.token
		//if(response.r.id != this.nodeID)
			//maybe remove missing node or findNode on it
		return [parsePeerContactInfos(response.r.values), parseNodeContactInfos(response.r.nodes)]
	}

	//used to announce
	async announcePeer() {
		let transactID = crypto.randomBytes(2)
		let request = {'t': transactID, 'y':'q', 'name': 'announce_peer', 'a' : {'id':this.myNodeID, 'infoHash':infoHash}}
		response = await this._sendRequest(request)
		//if(response) this.resetQueryTimer()
		//target id, token, nodes or values
		return response.r
...
//single string
var parseNodeContactsInfos = function parseNodeInfos(compactInfos) {
	return compactInfos.match(/.{26}/g).map((info)=> {return makeNode(info.slice(0,20), info.slice(24,26), info.slice(20,24))})
}
