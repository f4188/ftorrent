...
function foobar() {  
	Array.from(client1.activePieces.values()).map(x => [x.index, 
		foo(x.piecelets.size, "#")  
		+ foo( Array.from(x.requestList.values()).reduce( (sum , req) => req.dispatched == 1 ? sum + 1 : sum , 0), '-') 
		+  foo(x.requestList.size,"-")]).forEach((x, i) => console.log("", ("    " + x[0]).slice(-5), "|",x[1])) }
